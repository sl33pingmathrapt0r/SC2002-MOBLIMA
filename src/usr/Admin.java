...
public class Admin extends User {
    ...
    public void createCinemaShowtimes(Cineplex cineplex) {
        while (true) {
            ...
            try {
                cineplex.addScreening(choice, title, Integer.valueOf(screen.substring(0, 4)),
                        Integer.valueOf(screen.substring(4)), typeOfMovie);
    ...
    public void updateCinemaShowtimes(Cineplex cineplex) {
        while (true) {
            ...
            for (int i = 0; i < screenList.size(); i++)
                try {
                    System.out.println((i + 1) + ". " +
                            new SimpleDateFormat("dd MMM yyyy HH:mm").format(
                                    new SimpleDateFormat("yyyyMMddHHmm").parse(String.valueOf(screenList.get(i)))));
            ...
            cineplex.updateScreeningShowtime(
                    cineplex.cinemaFinder(title, screenList.get(choice - 1) % 1000, screenList.get(choice - 1) / 1000),
                    title, screenList.get(choice - 1) % 1000, Integer.valueOf(newScreen.substring(0, 5)),
                    screenList.get(choice - 1) / 1000, Integer.valueOf(newScreen.substring(5)));
        }
    }
